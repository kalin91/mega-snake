...
class VscodeLaunch(Enum):
    ...
    DEBUG_JAVA = ("JAVA DEBUG (Attach)", "java", "attach", None, None, {"port": 5005, "hostName": "localhost", "projectName": SUBSTITUTE_PROJECT_TAG})
    DEBUG_PYTHON_FILE = ("PYTHON DEBUG (File)", "debugpy", "launch", None, None, {"program": "${file}"})
    DEBUG_PYTHON_MODULE = (
        "PYTHON DEBUG (Module)",
        "debugpy",
        "launch",
    ...
    def __init__(self, task_name: str, task_type: str, request: str, env: Optional[dict[str, str]], args: Optional[list[str]], extra_args:Optional[dict[str, Any]]):
    ...
    def add_launch_config(self, json_data: dict[str, Any], string_substituter:Callable[[str],str]) -> Optional[dict[str, Any]]:
        """Adds the query to the workspace settings."""
        ...
        jq_query = f"{LAUNCH_CONFIG_QUERY} += [{string_substituter(json.dumps(self.to_dict()))}]"
