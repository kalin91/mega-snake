...
from mega_snake.util import props as props_module
...
def test_get_validated_input_proxy_delegates_to_util_module() -> None:
    ...
    with patch("mega_snake.util.util.get_validated_input", return_value="bash") as util_input:
...
@pytest.mark.parametrize(
    ...
    [
        (ModuleNotFoundError("missing module"), "Cannot access package resources for module 'mega_snake'"),
        ("", "Cannot access package resources for module 'mega_snake'"),
    ],
)
def test_init_app_properties_wraps_package_root_errors(files_side_effect: object, match: str) -> None:
    ...
    with patch("mega_snake.util.props.files") as files_mock:
...
def test_init_app_properties_falls_back_between_powershell_variants(
    ...
    with patch("mega_snake.util.props._get_package_root", return_value="/pkg"), patch(
        "mega_snake.util.props.os.path.exists", return_value=True
    ), patch("mega_snake.util.props._read_properties", return_value=properties), patch(
        "mega_snake.util.props.shutil.which",
        side_effect=lambda shell: (
            None if shell == requested_shell else f"/bin/{shell}" if shell == fallback_shell else None
        ),
    ), patch("mega_snake.util.props.AppProperties", return_value=app_properties) as app_properties_class, patch(
        "mega_snake.util.props.formatting.config_log"
    ), patch("mega_snake.util.props.formatting.ws_advice"):
...
def test_init_app_properties_rejects_invalid_shell_values_during_validation(tmp_path: Path) -> None:
    ...
    try:
        ...
        ), patch("mega_snake.util.props.shutil.which", return_value="/bin/fish"), patch(
            "mega_snake.util.props._find_code_workspace_files", return_value="workspace.code-workspace"
        ), patch("mega_snake.util.props.formatting.config_log"), patch(
            "mega_snake.util.props.formatting.ws_advice"
        ):
